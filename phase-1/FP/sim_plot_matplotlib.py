...
def maccskeys_fp(SMILES, Library):
    ...
    MACCKeys = [
        DataStructs.FingerprintSimilarity(y, x)
        for x, y in it.combinations(fps_MACCKeys, 2)
    ]
    MACCKeys.sort()
    sim = MACCKeys
...
class PlotSimPlt:
    def __init__(self, input_file):
        ...
        self.Data = pd.DataFrame.sample(
            self.Data, frac=0.5, replace=True, random_state=1992, axis=None
        )
...
sim_data = a.storage_sim_data(atom_pairs_fp)
a.plot_sim(sim_data, "AtomPairs")
a.stats(sim_data, "AtomPairs")
